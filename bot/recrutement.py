...
class Recrutement(commands.Cog):
    ...
    async def create_recrue(self, ctx: discord.ApplicationContext, user: Option(discord.User, "Entre un utilisateur.", required=True), pseudo: Option(str, "pseudo IG.", required=True), age: Option(int, "Age.", required=False)):
        ...
        if temp is not None:
            await ctx.respond(f"{user.mention} est déjà dans la base de donnée diplomatique.")
            cur.close()
            return
        cur.close()
        ...
        if temp is not None:
            await ctx.respond(f"{user.mention} est déjà dans la base de donnée de pays.")
            cur.close()
            return
        ...
        await user.add_roles(ctx.guild.get_role(config["roles"]["grades"]["nouvelle_recrue"]))
    ...
    async def create_diplomate(self, ctx: discord.ApplicationContext, user: Option(discord.User, "Entre un utilisateur.", required=True), pseudo: Option(str, "pseudo IG.", required=True), relation: Option(str, "Niveau de relation.", choices=["Neutre", "Allié", "Ami"],required=False, default="Neutre"), médaille: Option(str, "Grades médailles.", choices=["Guide", "Modo", "OP (SuperModo/Admin)"],required=False)):
        ...
        if "error" in response.json():
            ...
            await user.add_roles(ctx.guild.get_role(config["roles"]["grades"]["link"]))
            text += f"<@&{config['roles']['grades']['link']}>"

        cur = self.bot.worlddb.cursor()
        cur.execute(
            "INSERT INTO diplomatie (id_discord, pseudo_ingame) VALUES (:id_discord, :pseudo_ingame)",
            data)
        ...
        if api_ok == 1:
            if len(f"Unlink | {data['pseudo_ingame']}") <= 32:
                await user.edit(nick=f"Unlink | {data['pseudo_ingame']}")
        ...
        if médaille == "Guide":
            await user.add_roles(ctx.guild.get_role(config["roles"]["grades"]["guide"]))
            text += f", <@&{config['roles']['grades']['guide']}>"
        elif médaille == "Modo":
            await user.add_roles(ctx.guild.get_role(config["roles"]["grades"]["modo"]))
            text += f", <@&{config['roles']['grades']['modo']}>"
        elif médaille == "OP (SuperModo/Admin)":
            await user.add_roles(ctx.guild.get_role(config["roles"]["grades"]["op_sm_admin"]))
            text += f", <@&{config['roles']['grades']['op_sm_admin']}>"
    ...
    async def remove_user(self, ctx: discord.ApplicationContext, pseudo: Option(str, "Entre un pseudo IG."), de: Option(str, "db de pays ou de diplomatie.", choices=["Pays", "Diplomatie"],required=False, default="Pays")):
                ...
                if cur.fetchone()[0] == 0:
                    ...
                    if user is not None:
                        ...
                        for role in config["roles"]["deco"]["global"]:
                            await user.add_roles(ctx.guild.get_role(role))
                        await user.add_roles(ctx.guild.get_role(config["roles"]["grades"]["neutre"]))
                        await user.add_roles(ctx.guild.get_role(config["roles"]["grades"]["reglement_valider"]))
                        if erreur == 0:
                            await ctx.respond(
                                f"<@{user_id}> a correctement été supprimer de la base de données.")
                        else:
    ...
    # Détection arriver nouveau membre OLD
